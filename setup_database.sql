-- Complete Database Setup Script for Computer Lab Inventory Management System (Single User Version)
-- This script creates the database, tables, and inserts sample data with only one admin account
...
CREATE DATABASE IF NOT EXISTS amp CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE amp;

-- ==============================================
-- TABLE CREATION (Drop existing tables first)
-- ==============================================
...
CREATE TABLE pcs (
    ...
    createdAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updatedAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (roomLocationId) REFERENCES room_locations(id),
    FOREIGN KEY (createdBy) REFERENCES accounts(id)
);

-- 10. PC Components table
...
-- 11. Specification fields table
CREATE TABLE specification_fields (
    id INT AUTO_INCREMENT PRIMARY KEY,
    categoryId INT NOT NULL,
    fieldName VARCHAR(100) NOT NULL,
    fieldType ENUM('text', 'number', 'select', 'boolean') NOT NULL DEFAULT 'text',
...
CREATE TABLE disposes (
    ...
    FOREIGN KEY (locationId) REFERENCES storage_locations(id),
    FOREIGN KEY (createdBy) REFERENCES accounts(id),
    FOREIGN KEY (returnedBy) REFERENCES accounts(id)
);
...
CREATE TABLE approval_requests (
    id INT AUTO_INCREMENT PRIMARY KEY,
    type ENUM('stock', 'dispose') NOT NULL,
    ...
    updatedAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (createdBy) REFERENCES accounts(id),
    FOREIGN KEY (approvedBy) REFERENCES accounts(id)
);
...
CREATE INDEX idx_stocks_itemId ON stocks(itemId);
CREATE INDEX idx_stocks_locationId ON stocks(locationId);
CREATE INDEX idx_stocks_createdBy ON stocks(createdBy);
CREATE INDEX idx_pcs_roomLocationId ON pcs(roomLocationId);
CREATE INDEX idx_pcs_status ON pcs(status);
CREATE INDEX idx_disposes_itemId ON disposes(itemId);
CREATE INDEX idx_disposes_locationId ON disposes(locationId);
CREATE INDEX idx_disposes_createdBy ON disposes(createdBy);
CREATE INDEX idx_disposes_disposalDate ON disposes(disposalDate);
...
CREATE INDEX idx_approval_requests_createdBy ON approval_requests(createdBy);
CREATE INDEX idx_approval_requests_status ON approval_requests(status);

-- ==============================================
-- SAMPLE DATA INSERTION (Single User Version)
-- ==============================================
...
-- Display table counts to verify data insertion
...
UNION ALL
SELECT 'room_locations', COUNT(*) FROM room_locations
UNION ALL
SELECT 'items', COUNT(*) FROM items
UNION ALL
SELECT 'stocks', COUNT(*) FROM stocks
UNION ALL
SELECT 'pcs', COUNT(*) FROM pcs
UNION ALL
SELECT 'pc_components', COUNT(*) FROM pc_components
UNION ALL
SELECT 'specification_fields', COUNT(*) FROM specification_fields
UNION ALL
SELECT 'disposes', COUNT(*) FROM disposes
UNION ALL
...
SELECT 'Sample Account:' as info;
SELECT id, firstName, lastName, email, role, status FROM accounts LIMIT 1;

SELECT 'Sample Items:' as info;
SELECT i.id, i.name, b.name as brand, c.name as category 
FROM items i 
JOIN brands b ON i.brandId = b.id 
JOIN categories c ON i.categoryId = c.id 
LIMIT 5;

SELECT 'Sample Stocks:' as info;
SELECT s.id, i.name as item, s.quantity, sl.name as location, s.price 
FROM stocks s 
JOIN items i ON s.itemId = i.id 
JOIN storage_locations sl ON s.locationId = sl.id 
LIMIT 5;
...
SELECT 'You can now start the Node.js application.' as next_step;
SELECT 'Login with: admin@example.com / admin123' as login_info;
